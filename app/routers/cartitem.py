...
def get_cartitems(db:Session=Depends(database.get_db),current_user=Depends(get_current_user)):
...
def get_cartitem(id:int,db:Session=Depends(database.get_db),current_user=Depends(get_current_user)):
...
def create_cartitem(cartitem:schemas.CartItemBase,db:Session=Depends(database.get_db),current_user=Depends(get_current_user)):
...
def delete_cartitems(id:int,db:Session=Depends(database.get_db),current_user=Depends(get_current_user)):
...
def cart_item_update(id:int,cartitem:schemas.CartItemUpdate,db:Session=Depends(database.get_db),current_user=Depends(get_current_user)):
