...
def cart_items(db:Session=Depends(database.get_db),current_user=Depends(get_current_user)):
    carts = db.query(models.Cart).all()
    return carts


@router.get('/{id}',status_code=status.HTTP_200_OK,response_model=schemas.CartResponse)
def cart_item(id:int,db:Session=Depends(database.get_db),current_user=Depends(get_current_user)):
...
def create_cart(cart:schemas.CartBase,db:Session=Depends(database.get_db),current_user=Depends(get_current_user)):
...
def delete_cart(id:int,db:Session=Depends(database.get_db),current_user=Depends(get_current_user)):
