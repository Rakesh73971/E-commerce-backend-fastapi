...
def get_prodct(id:int,db:Session=Depends(database.get_db),current_user=Depends(get_current_user)):
...
def create_product(product:schemas.Product,db:Session=Depends(database.get_db),current_user=Depends(get_current_user)):
...
def delete_product(id:int,db:Session=Depends(database.get_db),current_user=Depends(get_current_user)):
...
def update_product(id:int,product:schemas.ProductUpdate,db:Session=Depends(database.get_db),current_user=Depends(get_current_user)):
