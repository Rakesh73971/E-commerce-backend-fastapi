...
def get_orderitems(db:Session=Depends(database.get_db),current_user=Depends(get_current_user)):
...
def get_orderitem(id:int,db:Session=Depends(database.get_db),current_user=Depends(get_current_user)):
...
def create_orderitem(orderitem:schemas.OrderItemBase,db:Session=Depends(database.get_db),current_user=Depends(get_current_user)):
...
def delete_orderitem(id:int,db:Session=Depends(database.get_db),current_user=Depends(get_current_user)):
    db_orderitem = db.query(models.OrderItem).filter(models.OrderItem.id == id).first()
    if db_orderitem is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail=f"orderitem with id {id} not found")
...
def update_orderitem(id:int,orderitem:schemas.OrderItemUpdate,db:Session=Depends(database.get_db),current_user=Depends(get_current_user)):
